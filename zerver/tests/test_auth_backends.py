...
class FetchAPIKeyTest(ZulipTestCase):
    ...
    def test_ldap_auth_email_auth_organization_restriction_fail(self) -> None:
        self.init_default_ldap_database()
    ...
    @override_settings(AUTHENTICATION_BACKENDS=('zproject.backends.ZulipLDAPAuthBackend',),
                       AUTH_LDAP_USER_ATTR_MAP={'full_name': 'cn', 'org_membership': 'department'})
    def test_ldap_auth_email_auth_organization_restriction_success(self) -> None:
        self.init_default_ldap_database()
    ...
    @override_settings(AUTHENTICATION_BACKENDS=('zproject.backends.ZulipLDAPAuthBackend',),
                       ...
                       AUTH_LDAP_ADVANCED_REALM_ACCESS_CONTROL={'zulip': {'test1': 'test', 'test2': 'testing'}, 'anotherRealm': {'test2': 'test2'}})
    ...
    def test_ldap_auth_email_auth_advanced_organization_restriction_wrong_configuration(self) -> None:
        self.init_default_ldap_database()
        # A wrong config/not existing realm should not grant any access
        self.change_ldap_user_attr('hamlet', 'department', 'zulip')
        result = self.client_post("/api/v1/fetch_api_key",
                                  dict(username=self.example_email('hamlet'),
                                       password=self.ldap_password('hamlet')))
